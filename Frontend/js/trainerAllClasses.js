let baseURL = "https://gofit-api.onrender.com";
...
function createRow(el) {
  return `<tr class="responsive-table__row">
      ...
      el.venue == "offline" ? el.locationOrLink : el.venue
		...
		<td class="responsive-table__body__text responsive-table__body__text--delete"><a href=./trainerSingleClass.html?id=${
      el._id
    }><button class="details-btn" >Details</button></a></td>
	</tr>`;
}
...
function searchalldata(event) {
  ...
  let temp = newData.filter(function (elem) {
    let ans =
      ...
      elem.title.toLowerCase().includes(searchdata.toLowerCase()) ||
...
function searchactivityfun(activity) {
  if (activity == "all") {
    ...
    let temp = newData.filter(function (elem) {
      let ans =
        elem.activity.toLowerCase().includes(activity.toLowerCase()) ||
